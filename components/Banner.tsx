...
import YouflexPlayer from './YouflexPlayer'
...
function Banner({ netflixOriginals }: Props) {
  ...
  const [playerOpen, setPlayerOpen] = useRecoilState(playerOpenState)
  ...
  useEffect(() => {
    fetch(
      'https://api.themoviedb.org/3/genre/movie/list?api_key=14e893e97ac3ae3e72bc8a4b529bf133&language=en-US'
    )
      .then((res) => res.json())
  ...
  return (
    <section className="banner-container fixed top-24 w-1/2 z-50">
      <h1 className="banner-title text-2xl font-bold md:text-4xl lg:text-7xl z-[99]">
      ...
      <AutoScrollParagraph>{currentMovie.overview}</AutoScrollParagraph>

      <div className="flex space-x-3 mt-4">
        <button className="bannerButton bg-white text-black" onClick={handlePlayClick}>
      ...
      {showPlayer && <YouflexPlayer onClose={handleClosePlayer} />}
    </section>
  )
}
